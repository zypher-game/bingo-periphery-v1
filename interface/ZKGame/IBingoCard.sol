...
interface IBingoCard {
    ...
    function getCardGame(
        uint256 cardId
    ) external view returns (address gameContract, uint256 gameId);

    function getCardNumbers(uint256 cardId) external view returns (uint8[][] memory cardNumbers);

    function matchedLines(uint256 cardId) external view returns (uint8 lineCount);
    ...
    */
    ...
    function editUnplayedCard(uint256 cardId, bytes calldata encryptedContent) external;

    function bindCardGame(uint256 cardId, address gameContract, uint256 gameId) external;
